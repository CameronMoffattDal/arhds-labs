...
clinic |>
  group_by(sex) |>
  summarise(
    across(c(age, sbp_mmhg, bmi), \(x) mean(x, na.rm = TRUE)),
    .groups = "drop"
  )
```
...
```{r}
paste0('The number of rows in this dataset are: ', datasaurus_dozen %>% nrow())
paste0(
  'The number of columns in this dataset are: ',
  datasaurus_dozen %>% ncol()
)
paste0('The variables are this dataset are: ')
datasaurus_dozen %>% colnames()
```
...
dino_pcor <- dino_data |> summarise(r = cor(x, y))
```
...
star_pcor <- star_data |> summarise(r = cor(x, y))
```
...
The summary stastics are very similar because the data was designed to have identical mean, std, and even p correlation for each dataset in both the x and y direction. This designed data highlights the challenges with not visulalizing the data, as very divergent data can give similar results. 
...
```{r}  
aq <- aq %>% mutate(month_name = format(ISOdate(2000, Month, 1), "%B"))

aq %>%
  group_by(Month) %>%
  summarise(
    Ozone_mean = mean(Ozone, na.rm = TRUE),
    Temp_mean = mean(Temp, rm.na = TRUE),
    across(everything(), ~ sum(!is.na(.)), .names = "complete_days_{.col}"),
  )
```

```{r}
ggplot(aq, aes(x = Temp, y = Ozone, colour = month_name)) +
  geom_point() +
  geom_smooth(method = 'lm') +
  labs(title = "Ozone compared to Temperature ")
```

Suggesting that as the tempurature increases the Ozone levels increase. 
